...
import { productsCarouselData } from "~/data/carouselData";
...
export function ProductsCarousel() {
  ...
  const handlePagination = (direction: "left" | "right") => {
    ...
    const newSlide =
      direction === "right"
        ? (slideRef.current + 1) % productsCarouselData.length
        : (slideRef.current - 1 + productsCarouselData.length) %
    ...
    <div className="absolute bottom-12 right-16">
      <div className="relative flex h-[400px] items-center">
        <div className="relative h-full">
          {productsCarouselData.map((item, index) => {
            const isActive = index === currentSlide;
            const offset =
              ((index - currentSlide + productsCarouselData.length) %
                productsCarouselData.length) *
              240;
          ...
          <div className="flex h-12 w-[320px] items-center">
            <div className="h-[3px] w-full bg-white/20">
              <div
                ...
                style={{
                  width: `${((currentSlide + 1) / productsCarouselData.length) * 100}%`,
                }}
              />
            </div>
          </div>
